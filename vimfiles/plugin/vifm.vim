...
"Gvim cannot handle ncurses so run vifm in an xterm.
	...
	"vim's clientserver.  This should probably be changed for windows
	" as I believe the clientserver is allways running in Windows.
	let s:f =  $APPDATA . "\\Vifm\\vimfiles" 

	let [s:r; s:rest]  =readfile( s:f, 1)
